...
$activeGw = (int)$array['active_gw'];//active_gw -> {1,2,...38,39(POSTSEASON)}
$pointsGw = (int)$array['points_gw'];
...
if(isset($_GET['userId'])){//if watching points of other user 
    $user_id = $_GET['userId'];
...
<html>
    <head>
        ...
        <script>
            ...
            function showPoints(xhttp){
                document.getElementById("playersPoints").innerHTML = 
                        xhttp.responseText;
            }
        </script>
    </head>
    <body onload="onLoadFunction()">
        ...
        if($playerFound){
        ?>
            <div id="playersPoints" class="points">
                ...
                if($registrationGw < $pointsGw){//button for previous GW
                    $q = ($pointsGw - 1)."|".$user_id;
                ?>
                <div class="buttons__gw">
                    <div class="item--previous"><button type='button' class="butt--full" onclick="loadDoc('displayPoints.php', showPoints, '<?php echo $q; ?>')">GW<?php echo ($pointsGw - 1)." "; ?>PTS</button></div>
                </div>
                <?php    
                }
                ?>
        ...
        <?php
            ...
            $msg = "No user with given ID in DB for GW".$pointsGw."!";
